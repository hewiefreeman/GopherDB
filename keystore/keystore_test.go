...
func TestChangeUniqueItem(t *testing.T) {
	if !setupComplete {
		t.Skip()
	}
	var guestName string = "guest" + strconv.Itoa(table.Size())
	err := table.UpdateKey(guestName, map[string]interface{}{"email": "changedEmail@gmail.com"})
	if err.ID != 0 {
		t.Errorf("TestChangeUniqueItem error: %v", err)
	}
	err = table.UpdateKey(guestName, map[string]interface{}{"email": guestName + "@gmail.com"})
	if err.ID != 0 {
		t.Errorf("TestChangeUniqueItem error: %v", err)
	}
}
...
func TestInsertDuplicateUniqueTableValue(t *testing.T) {
	...
	if err.ID != helpers.ErrorKeyInUse {
